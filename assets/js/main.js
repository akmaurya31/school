...
$(function () {
  ...
  MINOVATE.navbar = {
    ...
    menu: function () {
      ...
      if ($dropdowns.length > 0) {
        ...
        var $activeSubmenu = $('.dropdown.submenu>ul>.active').parent();
  ...
  var $window = $(window),
    ...
    $dropdowns = $navigation.find('ul').parent('li').find('ul').parent('li'),
...
$(function () {
  $('button[type="submit-new"]').on('click', (e) => {
    swal({
      title: "Data Saved!",
...
$(function () {
  $('#feeHeadTxt').keyup(function (e) {
    var txtVal = $(this).val();
    $('#displayTxt').attr("value", txtVal);
  });
});
/* fee allocation dynamic data view */
$(function () {
  $('#fee-allocate-grpName').hide();
  $('#fee-allocate-forBatch').hide();
  $('#fee-allocate-forStudent').hide();
  ...
  $('#feeForSel').change(() => {
    if ($('#feeForSel option:selected').val() == 'Selected Course/Batch/Section') {
      $("#fee-allocate-forBatch").show();
    }
    if ($('#feeForSel option:selected').val() == 'Selected Student') {
      $("#fee-allocate-forStudent").show();
    }
  });

  //selecting types of course/batch/section
  $("#fee-allocate-forStudent #courseSel").change(() => {
    var exists = $("#courseSel option[value='-1']").length !== 0;
    if (exists) {
      $('#batchSel').attr('disabled', false);
    }
  });
  $("#fee-allocate-forStudent #batchSel").change(() => {
    var exists2 = $("#batchSel option[value='-1']").length !== 0;
    if (exists2) {
      $('#sectionSel').attr('disabled', false);
    }
  });
  $("#fee-allocate-forStudent #sectionSel").change(() => {
    $('#fee-allocate-tbl--details').show();
  });
});

/* paymnet mode in history paymode option enabled/disbale */
$(function () {
  $('#payModeSelDiv').hide();
  $('#chequeNoDiv').hide();
  $('#refNoDiv').hide();
  ...
  $('#bankNameDiv').hide();
  $("#totFee").keyup(() => {
    if ($("#totFee").val().length > 1) {
      //$('#payModeSel').attr('disabled', false); 
      $('#payModeSelDiv').show();
    } else {
      $('#payModeSelDiv').hide();
    }
...
$(function () {
  $('#ctcInp').focusout(() => {
    var basciInp = 0.5 * $('#ctcInp').val();
    $('#basicInp').val(basciInp);
    var pfInp = 0.12 * $('#basicInp').val();
    $('#pfInp').val(pfInp);
  });
  $('#basicInp').focusout(() => {
    var hraInp = 0.5 * $('#basicInp').val();
    $('#hraInp').val(hraInp);
  });

  /* adding a input on add button click for allowance */
  var x = 1;
  var max_x_field = 3;
  $('#addRowSpcAll').on('click', (e) => {
    e.preventDefault();
    if (x < max_x_field) {
      x++;
      $('.wrapper-spcAll').append('<div><input class="form-control dynamicInp spcAllncInp sumAllowParam form-group" type="number" required ><i class="delRow fa fa-trash fa-2x" aria-hidden="true"></i></div>');
    }

  });

  $('.wrapper-spcAll').on("click", ".delRow", function (e) { //user click on remove text
    e.preventDefault();
    $(this).parent('div').remove(); x--;
  })

  /* adding a input on add button click for deduction */
  var y = 1;
  var max_y_field = 3;
  $('#addRowAdvSal').on('click', (e) => {
    e.preventDefault();
    if (y < max_y_field) {
      y++;
      $('.wrapper-deductAll').append('<div><input class="form-control dynamicInp advSalaryInp sumDeductParam form-group" type="number" required ><i class="delRow fa fa-trash fa-2x" aria-hidden="true"></i></div>');
    }

  });


  $('.wrapper-deductAll').on("click", ".delRow", function (e) { //user click on remove text
    e.preventDefault();
    $(this).parent('div').remove(); y--;
  })

  /* adding sum of the values */

  $(document).on('change', '.sumParam', () => {
    var sum = 0;
    $('.sumParam').each(function () {
  ...
  $(document).on('change', '.sumDeductParam', () => {
    var sum = 0;
    $('.sumDeductParam').each(function () {
      sum += parseFloat($(this).val());
    });
    //console.log("total", sum);
    $('#totDeductionInp').val(sum);
  });
