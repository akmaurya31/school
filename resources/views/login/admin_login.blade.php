...
<h2 class=" header-text"><img src="{{ asset('assets/images/loginicon_superadmin.png')}}"
class="login-icon" />Log In <span>Admin </span></h2>
</div>
...
<form name="form" class="form-validation" novalidate="" method='post' action="{{ route('login.admin-login-request') }}">
{{ csrf_field() }}
<div class="form-group ">
<div class="input-group input-group-icon">
    <span class="input-group-addon">
    <span class="icon">
        <i class="far fa5-user"></i>
    </span>
</span>

<input type="email" class="form-control @error('email') is-invalid @enderror"" id="eml" name='email' placeholder="Email Address" value="{{ old('email') }}">
                 @error('email')
...
<div class="form-group">
    <div class="input-group input-group-icon">
        ...
        <input type="password" id="pass" placeholder="Password" name='password' class="form-control @error('password') is-invalid @enderror">        
        @error('password')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
...
<label class="pull-right checkbox checkbox-custom-alt checkbox-custom-sm inline-block"><input
type="checkbox"><i></i>Remember me </label>
<div class=""><a href="#" class="lostpass">Forgot Password?</a></div>
</div>

<div class="form-group">
    <button type="submit" id="" class="btn btn-block btn-round">
        <i class="fas fa5-sign-in-alt"></i> Login
    </button>
</div>
...
$(document).ready(function() {
    $("body").backgroundSlideshow({
        ...
        fixed: true,
        images: ["assets/images/login-sadmin1.jpg", "assets/images/login-sadmin2.jpg",
            "assets/images/login-sadmin3.jpg"
        ]
    })
})
</script>
