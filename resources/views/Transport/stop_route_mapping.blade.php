...
<li>
<a href="javascript:void(0)">Stop Route Mapping</a>
</li>
</ul>
...
<div class="row">
  <div class="col-md-12">
    ...
    <strong>Stop</strong> Route Mapping</h1>
    ...
    <div class="table_ws_nowrap">
       <table class="table table-bordered table-responsive mb-0 data_tbl--feature">
        <thead>
          <tr>
            <th class="stl_th1"></th>
            <th class="stl_th2">Route</th>
            <th class="stl_th3">Vehicle</th>
            <th class="stl_th4">Stop</th>
            <th class="stl_th5">Time</th>
            <th class="stl_th6 action_icons_th">Actions</th>
            <th class="stl_th7">Priority</th>
          </tr>
        </thead>
        <tbody>
            @foreach($mapping as $key=>$val)
                  <tr>
                    ...
                    <td>
                      ...
                      <button class="btn badge badge-warning"  onclick="return delete_route_mapping({{ $val->id}});" data-toggle="tooltip" title="Delete"><i class="fa fa-trash fa-2x" aria-hidden="true"></i></button>
                    </td>  
                    <td><span class="status badge badge-secondary">{{$val->priority}}</span></td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="ak_tile_subhead"> <h2> Create Route Mapping</h2></div>
      </div>
      <form role="form"  method="post" action="{{route('admin.add_route_mapping')}}" class="box">
           {{ csrf_field() }}
        <div class="modal-body row">
          ...
          <div class="form-group col-sm-6 col-xs-12">
            ...
            <!-- <input class="form-control" type="number"  onkeydown="limit(this,12);" onkeyup="limit(this,12);" name="priority" id="priority" placeholder="Eg: 1" required> -->
            <input class="form-control" type="text" name="priority" id="priority" placeholder="Eg: 1" required>

          </div>
          <div class="form-group col-sm-6 col-xs-12">
            <label for="time">Time</label>
            <input class="form-control" type="time" name="time" id="time" placeholder="Eg: 88" required>
          </div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="ak_tile_subhead"> <h2> Update Route Mapping</h2></div>
      </div>
      <form role="form"  method="post" action="{{route('admin.update_route_mapping')}}" class="box">
           {{ csrf_field() }}
        <div class="modal-body row">
          <div class="form-group col-sm-6 col-xs-12">
      ...
      <div class="modal-body">
        <div class="quickvie-holder">             
              <div class="quick_info">
                  <ul>
                      <li class="qi-item id-1"><b>Route:</b> <span id="route">Tata Bus</span></li>
                      <li class="qi-item id-1"><b>Vehicle:</b> <span id="veh_name">DL 04 JC 6789</span></li>
                      <li class="qi-item id-1"><b>Stop:</b> <span id="sto">DL 04 JC 6789</span></li>
                      <li class="qi-item id-2"><b>Priority:</b> <span id="prior">22 Aug 2020</span></li>
                      <li class="qi-item id-3"><b>Time:</b> <span id="stop_time">21 Aug 2021</span></li>
...
function delete_route_mapping(i){
...
if(confirm)
{
    ...
    $.ajax({                        
        ...
        url: '{{ route("delete_route_mapping") }}',                      
        ...
        success: function (argument) {  
          swal('success','This Vehicle permit deleted successfully','success').then((value)=>{
                  location.reload();
                    });
...
function edit_details(i){
	    	$.ajax({                        
					url: '{{ route("admin.edit_route_mapping") }}',                      
					type: 'GET',                       
					data: {id:i},      
					dataType: 'json',                       
            ...
            $("#formModal2 #stop").append(data.stop);
					    ...
					    $("#formModal2 #route_name").val(data.data['route']);
					   
					    $("#formModal2 #stop").val(data.data['stop']);
					    $("#formModal2 #priority").val(data.data['priority']);
					    $("#formModal2 #time").val(data.data['time']);
...
function view_details(i){
	    	$.ajax({                        
					url: '{{ route("admin.show_route_mapping")}}',                      
					type: 'GET',                       
					data: {id:i},      
					dataType: 'json',                       
					success: function (data) { 
              ...
              $("#route").html(data.data['route']);
					    $("#veh_name").html(data.data['vehicle']);
					    $("#sto").html(data.data['stop']);
					    $("#prior").html(data.data['priority']);
					    $("#stop_time").html(data.data['time']);
